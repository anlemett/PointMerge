...
def create_dataset(month, week):
    ...
    input_filename1 = "osn_EIDW_states_50NM_2019_10_week" + str(week) + "_rwy28L.csv"
    
    states_df = pd.read_csv(os.path.join(DATA_INPUT_DIR, input_filename1), sep=' ',
    ...
    input_filename2 = "week" + str(week)+ "_50NM_more_than_25_flights_per_hour_flight_ids.txt"
    flight_ids_list = open(os.path.join(DATASET_DIR, input_filename2),'r').read().split('\n')
    ...
    dataset_df.to_csv(os.path.join(DATASET_DIR, filename), sep=' ', encoding='utf-8', float_format='%.3f', index = True, header = False)
