...
def create_dataset(month, week):
    ...
    number_of_trombone_flights = len (flight_ids_list)
    print(number_of_trombone_flights)
    count = 0

    trombone_states_df = pd.DataFrame()

    for flight_id in flight_ids_list:
        count = count + 1
        print(number_of_trombone_flights, count)
    
        flight_df = states_df[states_df.index.get_level_values('flightId') == flight_id]
    
        trombone_states_df = trombone_states_df.append(flight_df)
    
        filename = "TB_dataset_week" + str(week) + ".csv"
        trombone_states_df.to_csv(os.path.join(DATASET_DIR, filename), sep=' ', encoding='utf-8', float_format='%.3f', index = True, header = False)
